function y= mask(x,m)
% mask aplica a la imagen de entrada x la máscara especificada en m, dando
% como resultado una imagen enmascarada.
% INPUT:
%     x: Imagen original
%     m: Máscara a aplicar
% OUTPUT:
%     y: Imagen resultado de aplicar la máscara a la imagen original
...
%     límites donde hay imagen
    for f = f1:fend
        for c = c1:cend
            y(f-Yc+1,c-Xc+1) = sum(sum(x(f-Yc+1:f+Yc-1,c-Xc+1:c+Xc-1).*m));
        end
    end
end
end
